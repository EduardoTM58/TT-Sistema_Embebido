...
unsigned char *abrirBMP(char *filename, bmpInfoHeader *bInfoHeader)
{
	...
	if( f  == NULL )
	{
  	...
  	imgdata = (unsigned char *)malloc( (bInfoHeader->width) *(bInfoHeader->height) * 3 );
	if( imgdata == NULL )
	{
  	...
  	fread(imgdata, (bInfoHeader->width) *(bInfoHeader->height) * 3,1, f);
...
void guardarBMP( char *filename, bmpInfoHeader *info, unsigned char *imgdata )
{
	...
	if( f  == NULL )
	{
  	...
  	header.size = ( (info->width) *(info->height) * 3) + sizeof(bmpFileHeader) + sizeof(bmpInfoHeader);
  	/* header.resv1=0; */
  	/* header.resv2=1; */
  	...
  	fwrite(imgdata, ((info->width) *(info->height) * 3 ), 1, f);
  	fclose(f);
}
